...
export default function PemasukanPage() {
    ...
    // Daftar Tahun (Dari 2025 sampai 10 tahun ke depan)
    ...
    const [dataPemasukan, setDataPemasukan] = useState<Pemasukan[]>([]);
    ...
    const handleDeleteClick = async (id: string) => {
        const confirmDelete = window.confirm("Apakah Anda yakin ingin menghapus data pemasukan ini? Tindakan ini tidak dapat dibatalkan.");
                        ...
                        <div className="p-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <div className="flex gap-3">
                                <select 
                                    ...
                                    className="bg-[#1c1c2a] text-teal-400 font-bold border border-white/10 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-teal-500 cursor-pointer hover:bg-[#252536] transition-all"
                                >
                                    {daftarBulan.map((b) => (
                        ...
                        <div className="overflow-x-auto flex-1 p-2">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {isLoading ? (
                                        <tr>
                                            <td colSpan={5} className="p-10 text-center text-teal-500/50 animate-pulse font-medium">
                                                Mengambil data bank data...
                                            </td>
                                        </tr>
                                    ) : dataPemasukan.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="p-10 text-center text-gray-500 italic">
                                                Tidak ada catatan pemasukan di bulan ini.
                                            </td>
                                        </tr>
                                    ) : (
                                        dataPemasukan.map((item) => (
                                            <tr key={item.id} className="hover:bg-white/[0.02] transition-colors text-sm border-b border-white/5 group">
                                                ...
                                                <td className="p-4 text-teal-400 font-bold whitespace-nowrap">{formatRupiah(item.jumlah)}</td>
                                                <td className="p-4 text-gray-300 max-w-xs truncate">{item.keterangan}</td>
                                                <td className="p-4">
                                                    <span className="bg-teal-500/10 text-teal-300 px-3 py-1 rounded-full border border-teal-500/20 text-xs font-semibold tracking-wide shadow-[0_0_10px_rgba(45,212,191,0.05)]">
                                                        {item.kategori}
                                                    </span>
                                                </td>
                                                <td className="p-4">
                                                    <div className="flex justify-center gap-3 opacity-100 sm:opacity-50 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        <button 
                                                            ...
                                                            className="text-rose-400 hover:text-rose-300 hover:scale-110 transition-transform p-1 bg-rose-500/10 rounded hover:bg-rose-500/20"
                                                            title="Hapus"
                                                        >
                        ...
                        <div className="mb-6 border-b border-white/5 pb-4 flex justify-between items-end">
                            <div>
                                <h2 className="text-xl font-bold text-white tracking-wide">
                                    {isEditing ? 'Update Data' : 'Catat Masuk'}
                                </h2>
                                <p className="text-xs text-gray-500 mt-1">
                                    {isEditing ? 'Perbaiki kesalahan input' : 'Tambahkan dana baru'}
                                </p>
                            </div>
                            {isEditing && (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full px-4 py-3 bg-[#0B0B0F]/50 border border-white/5 rounded-xl text-white focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500/50 transition-all [color-scheme:dark]"
                                />
                            </div>

                            <div>
                                <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide ml-1">Sumber Dana</label>
                                <select 
                                    value={kategori}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        placeholder="0"
                                        className="w-full pl-10 pr-4 py-3 bg-[#0B0B0F]/50 border border-white/5 rounded-xl text-teal-400 font-bold focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500/50 transition-all placeholder-gray-700"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    placeholder="Contoh: Gaji bulan maret dari PT. ABC..."
                                    rows={3}
                                    className="w-full px-4 py-3 bg-[#0B0B0F]/50 border border-white/5 rounded-xl text-white text-sm focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500/50 resize-none transition-all placeholder-gray-600"
                                ></textarea>
                            </div>
                            ...
                            <button 
                                ...
                                className={`relative w-full overflow-hidden group px-6 py-4 rounded-xl font-bold transition-all duration-300 hover:-translate-y-1 active:scale-95 shadow-lg mt-2
                                    ...
                                    : isEditing 
                                        ...
                                        : 'bg-gradient-to-r from-teal-500 to-emerald-500 text-white shadow-[0_0_20px_rgba(20,184,166,0.3)] hover:shadow-[0_0_30px_rgba(20,184,166,0.5)]'}`}
                            >
                                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-in-out"></div>
                                <span className="relative z-10 tracking-wide">
                                    {isSaving ? 'Memproses...' : isEditing ? 'Update Perubahan' : '+ Tambah Pemasukan'}
                                </span>
                            </button>
                        </form>
                    </div>
