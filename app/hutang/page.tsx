...
export default function HutangPage() {
    ...
    const handleDeleteClick = async (id: string) => {
        const confirmDelete = window.confirm("Apakah Anda yakin ingin menghapus catatan hutang ini?");
    ...
    return (
        <div className="relative min-h-screen bg-[#0B0B0F] text-white p-4 sm:p-6 font-sans overflow-x-hidden">
            ...
            {/* Background Glow Effects (Cyan/Indigo Theme) */}
                ...
                <header className="relative flex items-center justify-between bg-[#151521]/70 backdrop-blur-xl p-4 sm:p-5 rounded-[2rem] shadow-xl border border-white/5">
                    ...
                    <div className="absolute left-0 w-full text-center pointer-events-none">
                        <h1 className="text-2xl font-extrabold tracking-tight">
                            Manajemen <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-400">Hutang</span>
                        </h1>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-1 bg-[#151521]/70 backdrop-blur-xl rounded-[2rem] border border-white/5 shadow-xl flex flex-col min-h-[500px]">
                        ...
                        <div className="p-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div>
                                <h3 className="text-lg font-bold text-white">Buku Catatan Hutang</h3>
                                <p className="text-sm text-gray-400">Arsip piutang & pinjaman bulanan</p>
                            </div>
                            ...
                            <div className="flex gap-3">
                                <select 
                                    ...
                                    className="bg-[#1c1c2a] text-cyan-400 font-bold border border-white/10 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-cyan-500 cursor-pointer hover:bg-[#252536] transition-all"
                                >
                                    {daftarBulan.map((b) => (
                        ...
                        <div className="overflow-x-auto flex-1 p-2">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {isLoading ? (
                                        <tr>
                                            <td colSpan={5} className="p-10 text-center text-cyan-500/50 animate-pulse font-medium">
                                                Membuka arsip catatan...
                                            </td>
                                        </tr>
                                    ) : dataHutang.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="p-10 text-center text-gray-500 italic">
                                                Bebas hutang di bulan ini! Pertahankan!
                                            </td>
                                        </tr>
                                    ) : (
                                        dataHutang.map((item) => (
                                            <tr key={item.id} className="hover:bg-white/[0.02] transition-colors text-sm border-b border-white/5 group">
                                                ...
                                                <td className="p-4 text-white font-bold whitespace-nowrap">{formatRupiah(item.jumlah)}</td>
                                                <td className="p-4 text-gray-300 max-w-xs truncate">{item.keterangan}</td>
                                                <td className="p-4 text-center">
                                                    {item.kategori === 'Masuk' ? (
                                                        <span className="bg-emerald-500/10 text-emerald-400 px-3 py-1 rounded-md border border-emerald-500/20 text-xs font-bold tracking-wide">
                                                            + MASUK
                                                        </span>
                                                    ) : (
                                                        <span className="bg-rose-500/10 text-rose-400 px-3 py-1 rounded-md border border-rose-500/20 text-xs font-bold tracking-wide">
                                                            - KELUAR
                                                        </span>
                                                    )}
                                                </td>
                    ...
                    <div className="w-full lg:w-[350px] flex flex-col gap-6">
                            ...
                            <div className="mb-6 border-b border-white/5 pb-4 flex justify-between items-end">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">
                                        {isEditing ? 'Perbaiki catatan hutang' : 'Masuk (Piutang) / Keluar (Pinjam)'}
                                    </p>
                                </div>
                                {isEditing && (
                                    <button onClick={resetForm} className="text-xs text-gray-400 hover:text-white underline decoration-gray-600 underline-offset-2">Batal</button>
                                )}
                            </div>
                            
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div>
                                    <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide ml-1">Tanggal</label>
                                    <input 
                                        type="date" 
                                        required
                                        value={tanggal}
                                        onChange={(e) => setTanggal(e.target.value)}
                                        className="w-full px-4 py-3 bg-[#0B0B0F]/50 border border-white/5 rounded-xl text-white focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 transition-all [color-scheme:dark]"
                                    />
                                </div>

                                <div>
                                    <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide ml-1">Tipe</label>
                                    <select 
                                        value={kategori}
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide ml-1">Keterangan / Nama</label>
                                    <textarea 
                                        required
                                        value={keterangan}
                                        onChange={(e) => setKeterangan(e.target.value)}
                                        placeholder="Contoh: Pinjam ke si A..."
                                        rows={2}
                                        className="w-full px-4 py-3 bg-[#0B0B0F]/50 border border-white/5 rounded-xl text-white text-sm focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 resize-none transition-all placeholder-gray-600"
                                    ></textarea>
                                </div>
                                ...
                                <button 
                                    ...
                                    className={`relative w-full overflow-hidden group px-6 py-4 rounded-xl font-bold transition-all duration-300 hover:-translate-y-1 active:scale-95 shadow-lg mt-2
                                        ...
                                        : isEditing 
                                            ...
                                            : 'bg-gradient-to-r from-cyan-600 to-indigo-600 text-white shadow-[0_0_20px_rgba(8,145,178,0.3)] hover:shadow-[0_0_30px_rgba(8,145,178,0.5)]'}`}
                                >
                        ...
                        <div className="bg-[#151521]/70 backdrop-blur-xl p-6 sm:p-8 rounded-[2rem] border border-white/5 shadow-xl flex flex-col gap-6">
                            <h3 className="text-white font-bold border-b border-white/5 pb-2">Ringkasan Hutang ({filterBulan}/{filterTahun})</h3>
                            
                            <div className="relative overflow-hidden group p-4 rounded-xl bg-gradient-to-br from-rose-500/10 to-transparent border border-rose-500/20">
                                <div className="absolute top-0 right-0 w-16 h-16 bg-rose-500/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-125"></div>
                                <p className="text-xs text-gray-400 mb-1 uppercase tracking-wider relative z-10">Total Hutang Keluar</p>
                                <p className="text-2xl font-bold text-rose-400 relative z-10">{formatRupiah(totalHutangKeluar)}</p>
                            </div>
                            
                            <div className="relative overflow-hidden group p-4 rounded-xl bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/20">
                                <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-125"></div>
                                <p className="text-xs text-gray-400 mb-1 uppercase tracking-wider relative z-10">Total Hutang Masuk</p>
                                <p className="text-2xl font-bold text-emerald-400 relative z-10">{formatRupiah(totalHutangMasuk)}</p>
                            </div>
                        </div>
